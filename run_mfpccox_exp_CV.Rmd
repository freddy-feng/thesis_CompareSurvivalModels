...
```{r}
tmp <- paste0("./model/model_", set_scenario, "_MFPCCox_", is_scaled, "_seed", seed, ".RData")
save(folds, file = tmp)
```
...
requires subject.id and multivar outside the training loop
...
#    filter(Years.bl <= T.start) # Filter long observations after landmark time
  ...
  for(p in 1:n.y){ # for each longitudinal covariate
    ...
    tmp.ufpca <- uPACE(
      ...
      predData = tmp.data[ , , p], 
      nbasis = 3,
      pve = pve) # run uPACE that wraps PACE and supply data
...
```{r}
tmp <- paste0("./output/eval_", set_scenario, "_MFPCCox_", is_scaled, "_seed", seed, ".RData")
