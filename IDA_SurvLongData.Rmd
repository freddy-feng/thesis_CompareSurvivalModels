...
example.raw <- ADNIMERGE::adnimerge %>%
  filter(RID == example.id) %>%
  select(RID, VISCODE, EXAMDATE, Years.bl, DX.bl, DX, AGE, PTGENDER, ADAS13) %>%
  head(3)
  

example.surv <- df.surv_preds %>%
  filter(RID == example.id) %>%
  select(RID, time, event, DX.bl, AGE, PTGENDER, ADAS13)

example.long <- df.long_censored %>%
  filter(RID == example.id) %>%
  select(RID, VISCODE, Years.bl, DX.bl, AGE, PTGENDER, ADAS13) %>%
  head(3)
...
table.transformation[3:4] <- table.transformation[3:4] %>%
  round(digits = 3)
colnames(table.transformation) <- c("Covariate", "Transformed covariate", "Skewness (After)", "Skewness (Before)")

table.transformation <- table.transformation %>%
  arrange(`Skewness (Before)`)

title.table <- "Transformation of longitudinal covariates"
latex(table.transformation, 
      digits = 3,
      title = title.table,
      caption = title.table,
      booktabs = TRUE,
