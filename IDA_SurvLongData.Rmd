...
source("function_utility_exp.R") # Utility functions
```
...
df.surv_preds <- df.surv_preds_prc
df.long <- df.long_censored_prc
```
...
g <- res.status %>%
  ggplot(aes(x = t, y = n, group = status, color = status)) +
    ...
    ggtitle("Cumulative number of subjects by status up to time t") +
  theme_bw()

g

ggsave(filename = "./figure/status_by_time.png", plot = g, width = 8, height = 5)
```
...
g <- df.surv_preds %>%
  ...
  ggtitle("Survival time by survival outcome") +
  labs(fill = "Type") + 
  xlab("Years since baseline (t)") +
  theme_bw()

g

ggsave(filename = "./figure/hist_survtime.png", plot = g, width = 8, height = 5)
```
