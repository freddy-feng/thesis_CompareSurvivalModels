...
```{r}
Load_evals <- function(folder = "./output/") {
   # Immediate output location of folds.eval

  models.path <- list.files(path = folder) # Using repeated measurements up to landmark time
...
folderpath <- "./output/"
out <- Load_evals(folderpath)
models.name <- out$models.name
models.path <- out$models.path
rm(out)
```
...
#    alpha = 0.3,
#    width = 0.3, size = 0.8) + 
  geom_line(aes(linetype = method), size = 0.5, alpha = 0.5) +
...
## Figure: Compare pencal and pCox | scenario 2
```{r, fig.width=8, fig.height=5}
# ------------------------------------------------------------------------------------------------
# Plot df.tdauc.compare
# ------------------------------------------------------------------------------------------------
...
```
## Figure: Compare pencal and pCox | scenario 2 or scenario 0 for pCox-bl
```{r, fig.width=8, fig.height=5}
# ------------------------------------------------------------------------------------------------
# Plot df.tdauc.compare
# ------------------------------------------------------------------------------------------------
df.tdauc.compare %>% 
  ...
  ggplot(aes(
    ...
    group = model.name,
    color = model.name
    # group = stringr::str_wrap(model.name, width = 20),
    # color = stringr::str_wrap(model.name, width = 20)
    )
  ) +
 # geom_errorbar(aes(
 #   ymax = repeated.CV.max,
 ...
 #   width = 0.3, size = 0.8) +
#  geom_line(aes(linetype = factor(n_bl.covariate)), alpha = 0.8, size = 0.4) +
#  geom_vline(xintercept = T.start, color = "grey80", linetype = "dashed", size = 1, alpha = 1) + # landmark
  geom_line(alpha = 0.8, size = 0.5) +
  geom_point(aes(shape = method), alpha = 0.8, size = 2.5) +
  scale_x_continuous(breaks = deltaT) + 
  scale_y_continuous(breaks = 0:10 / 10) +
...
## Figure: Compare pencal and pCox | scenario2 or scenario1 for MFPCCox  or scenario 0 for pCox-bl
```{r, fig.width=8, fig.height=5}
# ------------------------------------------------------------------------------------------------
# Plot df.tdauc.compare
# ------------------------------------------------------------------------------------------------
...
df.c.index.compare %>%
  # filter(grepl(model, pattern = "scaled")) %>% # dumb method to filter based on model.name
...
## Figure: Compare pencal and pCox
```{r, fig.width=8, fig.height=5}
# ------------------------------------------------------------------------------------------------
# Plot df.tdauc.compare
# ------------------------------------------------------------------------------------------------
...
#  ggsci::scale_color_jama() +
  ...
  labs(
    title = "Comparison of estimated BS under repeated 10-fold CV",
    subtitle = paste("at landmark time", T.start),
    caption = "CV repeated in 10 different seeds",
#    caption = "Error bar represents the max and min",
#    linetype = "number of (candidate) baseline covariates",
...
# Variable importance (require folds)




only usable when folds are kept, and using cv.glmnet

```{r, fig.width=4,  fig.height=8}
temp.min <- lapply(1:length(folds), function(i) {
  out <- Find.var.importance(folds[[i]]$model$step3$pcox.orig, lambda = "lambda.min")
  out <- data.frame(out)
  out <- cbind(var = rownames(out), out)
  out <- out %>% mutate(fold = i)
  rownames(out) <- NULL
  return(out)
  })
...
```{r, fig.width=4,  fig.height=8}
temp.1se <- lapply(1:length(folds), function(i) {
  out <- Find.var.importance(folds[[i]]$model$step3$pcox.orig, lambda = "lambda.1se")
  out <- data.frame(out)
  out <- cbind(var = rownames(out), out)
  out <- out %>% mutate(fold = i)
  rownames(out) <- NULL
...
temp.min %>%
  add_row(temp.1se) %>%
  ggplot(aes(x = coef, y = var, group = lambda, shape = lambda, color = lambda)) +
...
temp.min %>%
  add_row(temp.1se) %>%
  ggplot(aes(x = coef, y = var, group = lambda, shape = lambda, color = factor(fold))) +
